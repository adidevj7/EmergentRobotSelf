...
Run:
  CUDA_VISIBLE_DEVICES=2 ./isaaclab.sh -p /home/adi/projects/CreativeMachinesAnt/Isaac/scripts/play_ant_force_load_splithead_v2.py \
    --task Isaac-Ant-Direct-v0 \
    --cfg_yaml /home/adi/projects/CreativeMachinesAnt/Isaac/cfg/rlg_play_sac_ant.yaml \
    --checkpoint /home/adi/projects/CreativeMachinesAnt/Isaac/checkpoints/Ant_walk_new_mk13_2025-10-02_09-37-30/manual_ep_1200000_for_play.pth \
    --steps 1000 \
    --video_dir /home/adi/projects/CreativeMachinesAnt/Isaac/videos \
    --headless --enable_cameras --rendering_mode quality

CUDA_VISIBLE_DEVICES=6 ./isaaclab.sh -p /home/adi/projects/CreativeMachinesAnt/Isaac/scripts/play_ant_force_load_splithead_v2.py \
    --task Isaac-Ant-Direct-v0 \
    --cfg_yaml /home/adi/projects/CreativeMachinesAnt/Isaac/cfg/rlg_play_sac_ant_150.yaml \
    --checkpoint /home/adi/projects/CreativeMachinesAnt/Isaac/checkpoints/WSJ_att60_Aspin6_2025-12-25_06-24-20/models/c002_b02_spin_plateau_2025-12-25_13-56-39_for_play.pth \
    --steps 1000 \
    --video_dir /home/adi/projects/CreativeMachinesAnt/Isaac/videos \
    --headless --enable_cameras --rendering_mode quality
...
def main():
    ...
    p = argparse.ArgumentParser("Force-load rl_games SAC checkpoint (split-head) and record video")
    ...
    # (optional safety) if you suspect obs-normalization mismatch, uncomment:
    ...
    cfg.params["load_path"] = ""  # make sure Runner does NOT auto-load
    ...
    # try a few common mapping rules
    rulesets = [
        [],  # identity
        [("_orig_mod.", "")],
    ...
    best = (0, None, None)  # (matches, rules, remapped_sd)
    ...
    if matched == 0:
        # Help: print a few target keys so we can refine mapping if needed
    ...
    # 8) Confirm split-head (out = 2*act_dim), without assuming _orig_mod exists
    ...
    for path in candidates:
        actor = get_attr_path(player.model, path)
        if actor is not None:
            break

    W = None
    if actor is not None and hasattr(actor, "trunk"):
        try:
            last = actor.trunk[-1]
    ...
    if W is None:
        # Fallback: scan modules for a Linear with out_features==2*act_dim and name contains 'actor'
    ...
    print(f"\n[check] actor head weight shape = {shape} (expected out={2*act_dim})")
    if W is not None and shape[0] == 2*act_dim:
        print("✅ [check] split head present (μ|logσ).")
    else:
        print("❗ [check] split head NOT detected as expected — actions may be wrong.")

    # 9) Rollout and record
    obs, info = env.reset()
    steps = 0
    with torch.inference_mode():
        while app.is_running() and steps < args.steps:
    ...
    env.close()
    app.close()
    print(f"\n[OK] Saved video(s) to: {video_dir.resolve()}  (prefix='{prefix}')")
