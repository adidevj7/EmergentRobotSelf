...
seed 0
CUDA_VISIBLE_DEVICES=5 ./isaaclab.sh -p /home/adi/projects/CreativeMachinesAnt/Isaac/scripts/Isaac_WSJ_att71_freeze.py \
    ...
    --num_envs 8192 --n_cycles 1 --phase_order walk \
    --updates_per_step 32 --plateau_min_steps 250_000_000 --max_steps_phase 1_500_000_000 \
    --override_warmup_steps 10000 --log_interval_s 30 --headless --lambda_back 1 --gpu 5 \
    --record_every 0 --phase_retry_max 0 --min_phase_mean_reward_on_switch 0 --restart_min_frac 1.0 \
    --resume_from /home/adi/projects/CreativeMachinesAnt/Isaac/analysis/SelfExtractorTriplets/models_all/run02_c019_b02_spin_plateau_2026-02-03_22-47-20_for_play.pth \
    --frozen_indices_json /home/adi/projects/CreativeMachinesAnt/Isaac/analysis/SelfExtractorTriplets/models_all/run02_c019_b02_spin_plateau_2026-02-03_22-47-20_self_freeze_idx.json \
    --run_tag att71_SelfFreeze_spinload_walk --ckpt_label att71_SelfFreeze_spinload_walk --seed 2
...
_GLOBAL_APP = None
_GLOBAL_FREEZER = None  # NeuronFreezer instance for current phase (None if no freeze)
...
p.add_argument("--frozen_indices_json", type=str, default=None,
               help="Path to *_self_freeze_idx.json or *_task_freeze_idx.json produced by "
                    "self_extractor_triplets.py. Freezes incoming weights of the specified "
                    "actor MLP neurons for the entire training run.")
...
try:
    ...
    def _hook_after_init(self, algo):
        ...
        Order of operations:
          1. Capture algo for logging/ckpts
          2. Load any queued incoming weights (resume_from)
          3. Attach NeuronFreezer AFTER weights are loaded, so hooks operate
             on the correct (loaded) weight values from the start
        """
        global _GLOBAL_ALGO, _GLOBAL_VECENV, _PENDING_LOAD_SD, _GLOBAL_FREEZER
        ...
        # ── Step 2: load queued incoming weights ─────────────────────────────
        # IMPORTANT: this must happen BEFORE attaching the freezer, so that
        # the frozen neurons start from the loaded checkpoint values, not from
        # the freshly-initialised random weights.
        if _PENDING_LOAD_SD:
            try:
                if hasattr(algo, "model") and hasattr(algo.model, "load_state_dict"):
                    ...
                    print(f"[weights] Queued incoming weights loaded into algo.model "
                          f"(strict=False). fp={fp}")
                else:
                    print("[weights] WARNING: algo.model not present; "
                          "could not load queued weights.")
            except Exception as e:
                print(f"[weights] WARNING: loading queued weights failed: {e}")
            _PENDING_LOAD_SD = None

        # ── Step 3: attach NeuronFreezer ─────────────────────────────────────
        frozen_by_layer = _resolve_frozen_indices()
        if frozen_by_layer:
            if _GLOBAL_FREEZER is not None:
                # Should not happen in normal flow, but clean up defensively
                _GLOBAL_FREEZER.remove()
            if hasattr(algo, "model") and algo.model is not None:
                _GLOBAL_FREEZER = NeuronFreezer(algo.model, frozen_by_layer, verbose=True)
            else:
                print("[freezer] WARNING: algo.model not yet available at hook time; "
                      "freezer not attached. Check rl_games version.")
                _GLOBAL_FREEZER = None
        else:
            _GLOBAL_FREEZER = None
            print("[freezer] No --frozen_indices_json provided; all neurons trainable.")
    ...
    print("[observer-hook] Hooked DefaultAlgoObserver.after_init "
          "(weights load + NeuronFreezer attach).")
...
# ──────────────────────────────────────────────────────────────────────────────
# 82. Neuron Freezer — gradient hooks for actor MLP hidden layers
# ──────────────────────────────────────────────────────────────────────────────

def _zero_rows(grad: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
    """Zero gradient rows for frozen neurons (incoming weights)."""
    if grad is None:
        return grad
    g = grad.clone()
    g[indices] = 0.0
...
def _zero_elements(grad: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
    """Zero gradient elements for frozen neuron biases."""
    if grad is None:
        return grad
    g = grad.clone()
    g[indices] = 0.0
...
class NeuronFreezer:
    """
    Freezes the INCOMING weights (weight rows + bias elements) of specific neurons
    in the actor MLP hidden layers, using gradient hooks.

    Outgoing weights are NOT frozen — the rest of the network can still freely
    re-weight the frozen neurons' outputs. This is the softer constraint:
    the neuron's learned representation is preserved but downstream can adapt.

    frozen_by_layer: {layer_rank: [neuron_indices]}
      layer_rank 0 = first  actor_mlp Linear layer (H1)
      layer_rank 1 = second actor_mlp Linear layer (H2)

    Layer ranks are assigned by sorting actor_mlp Linear layers by their integer
    index in the module name — exactly matching how _discover_actor_mlp_layers()
    works in the analysis script.
    """
    def __init__(self, model: torch.nn.Module,
                 frozen_by_layer: dict,
                 verbose: bool = True):
        self._hooks: list = []
        self._frozen_by_layer = {
            int(k): torch.tensor(v, dtype=torch.long)
            for k, v in frozen_by_layer.items() if v
        }
        self._n_hooks = 0
        self._attached_layers: dict = {}  # rank -> layer name (for logging)
        self._attach(model, verbose)

    def _attach(self, model: torch.nn.Module, verbose: bool):
        import re as _re

        # Collect all Linear layers under actor_mlp, sorted by their position index
        actor_linears = []
        for name, module in model.named_modules():
            if not isinstance(module, torch.nn.Linear):
                continue
            if "actor.trunk" not in name:
                continue
            # Parse the integer at the end of the actor_mlp subpath
            # e.g. "a2c_network.actor_mlp.0" → 0
            #      "model.actor_mlp.2"        → 2
            m = _re.search(r"actor\.trunk\.(\d+)$", name)
            if m:
                ...
                try:
                    ...
                    layer_num = 10 ** 6
            actor_linears.append((layer_num, name, module))

        actor_linears.sort(key=lambda t: t[0])

        if not actor_linears:
            if verbose:
                print("[freezer] WARNING: no actor_mlp Linear layers found in model — "
                      "check that model is fully initialised before NeuronFreezer is created.")
            return

        if verbose:
            print(f"[freezer] Found {len(actor_linears)} actor_mlp Linear layers: "
                  + ", ".join(f"rank{r}='{n}' {tuple(m.weight.shape)}"
                              for r, (_, n, m) in enumerate(actor_linears)))

        for rank, (_, name, module) in enumerate(actor_linears):
            if rank not in self._frozen_by_layer:
                continue

            idx = self._frozen_by_layer[rank].to(module.weight.device)

            # Guard: clip indices to valid range
            n_out = module.weight.shape[0]
            valid = idx[idx < n_out]
            if len(valid) < len(idx) and verbose:
                print(f"[freezer] rank={rank} '{name}': clipped {len(idx) - len(valid)} "
                      f"out-of-range indices (layer has {n_out} neurons).")
            idx = valid

            if idx.numel() == 0:
                continue

            # Hook on weight matrix rows (incoming weights to frozen neurons)
            h_w = module.weight.register_hook(
                lambda g, i=idx: _zero_rows(g, i)
            )
            self._hooks.append(h_w)

            # Hook on bias elements
            if module.bias is not None:
                h_b = module.bias.register_hook(
                    lambda g, i=idx: _zero_elements(g, i)
                )
                self._hooks.append(h_b)

            self._attached_layers[rank] = name

            if verbose:
                print(f"[freezer] rank={rank} '{name}' shape={tuple(module.weight.shape)} "
                      f"→ freezing {idx.numel()} neurons "
                      f"(incoming weights + bias; outgoing weights remain trainable)")

        self._n_hooks = len(self._hooks)
        if verbose:
            if self._n_hooks > 0:
                print(f"[freezer] Attached {self._n_hooks} gradient hooks across "
                      f"{len(self._attached_layers)} layers: {list(self._attached_layers.values())}")
            else:
                print("[freezer] WARNING: hooks requested but none were attached "
                      "(frozen_by_layer ranks may not match any actor_mlp layer ranks).")

    def remove(self):
        """Detach all gradient hooks — all neurons become trainable again."""
        for h in self._hooks:
            try:
                ...
                pass
        self._hooks.clear()
        print(f"[freezer] Removed {self._n_hooks} gradient hooks — all neurons now trainable.")

    def __repr__(self):
        return (f"NeuronFreezer(layers={list(self._attached_layers.values())}, "
                f"n_hooks={self._n_hooks})")


def _resolve_frozen_indices() -> dict:
    """
    Reads --frozen_indices_json and returns {layer_rank: [neuron_indices]}.
    Returns empty dict if arg not provided, file missing, or file has no valid keys.

    The JSON is expected to have keys "layer_0" and/or "layer_1" containing
    lists of original neuron indices — exactly the format produced by
    self_extractor_triplets.py.
    """
    json_path = getattr(args, "frozen_indices_json", None)
    if not json_path:
        return {}

    json_path = str(json_path).strip()
    if not os.path.exists(json_path):
        print(f"[freezer] WARNING: --frozen_indices_json path not found: {json_path}")
        return {}
    ...
    try:
        ...
        print(f"[freezer] WARNING: could not parse frozen_indices_json: {e}")
        return {}

    result = {}
    freeze_type = data.get("freeze_type", "unknown")
    tau         = data.get("tau", "?")
    print(f"[freezer] Loading frozen indices: type={freeze_type} tau={tau} src={json_path}")

    for layer_rank in [0, 1]:
        key = f"layer_{layer_rank}"
        if key not in data or not data[key]:
            continue
        indices = [int(x) for x in data[key]]
        n_total   = data.get(f"{key}_n_total",   "?")
        n_alive   = data.get(f"{key}_n_alive",   "?")
        n_self    = data.get(f"{key}_n_self_pool","?")
        n_task    = data.get(f"{key}_n_task_pool","?")
        n_frozen  = len(indices)
        result[layer_rank] = indices
        print(f"[freezer]   layer_{layer_rank}: n_frozen={n_frozen} | "
              f"total={n_total} alive={n_alive} self_pool={n_self} task_pool={n_task}")

    if not result:
        print(f"[freezer] WARNING: JSON loaded but no layer_0/layer_1 keys found.")
...
def _run_phase(
    ...
    """
    import gc
    global _GLOBAL_ALGO, _GLOBAL_VECENV, _ROLLOUT_LOG_PATH, _GLOBAL_ENV, _PHASE_RUNTIME, _PENDING_LOAD_SD, _LAST_FOR_PLAY, _LAST_STOP_REASON, _LAST_PLATEAU_MU, _GLOBAL_FREEZER
    ...
    try:
        ...
        # ── Remove neuron freezer before algo teardown ────────────────────────
        # Hooks hold references to parameter tensors; remove them before the
        # algo/model is GC'd to avoid dangling references.
        if _GLOBAL_FREEZER is not None:
            try:
                _GLOBAL_FREEZER.remove()
            except Exception:
        ...
        _GLOBAL_ALGO = None       # ← this line already exists; insert above it
        _PENDING_LOAD_SD = None
        runner = None
